...
# create new column for tallied features
finalCombined <- finalCombined %>% mutate(tally = 0)
View(finalCombined)
...
computed_mean <- function(column) {
  ...
  #l <- paste(column, ">",weighted_mean)
  #finalCombined <- finalCombined %>% filter_(l) %>% mutate(tally=tally+1)
  #finalCombined <- finalCombined %>% mutate_(tally = ifelse(column > weighted_mean, tally+1, tally))
  fltr <- response > weighted_mean
  finalCombined[fltr,"tally"] <- finalCombined[fltr,"tally"] + 1
...
for(i in 1:length(features)){
  ...
  finalCombined <- computed_mean(column_name)
}
#View weighted means for top 11 features
